...
export function PackingLoadingForm({
  ...
  const { data: checkData } = useQuery({
    queryKey: ["packing-loadings-check", invoiceIds],
    queryFn: () => packingChecksApi.checkLoading(invoiceIds),
  ...
  useEffect(() => {
    if (!isEditing && takenInvoiceMap.size > 0) {
      setSelectedInvoiceIds((prev) => {
        ...
        if (blocked.length > 0) {
          toast.warning(
            `${blocked.length} hóa đơn đã có lấy hàng, đã tự động bỏ khỏi danh sách`
          );
