...
import { usePromisedHNDrilldownByProduct } from "@/lib/hooks/useTransfers";
...
export function PromisedHNDrilldownModal({
  productId,
  productName,
  productSku,
  onClose,
  ...
  const { data, isLoading, isError, error, refetch } =
    usePromisedHNDrilldownByProduct(productId);
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-auto">
          {isLoading ? (
            ...
            <div className="py-16 text-center text-sm text-gray-400">
              Không có đơn Phiếu tạm hoặc Đã xác nhận tại Kho Hà Nội.
            </div>
          ) : (
        ...
        <div className="flex items-center justify-between border-t bg-gray-50 px-5 py-3 text-xs text-gray-600">
          <span>
            {records.length > 0
              ? `${data?.total ?? records.length} đơn — Tổng hứa bán HN: ${formatWholeQuantity(data?.sumQuantity ?? 0)}`
              : ""}
          </span>
          <button
            type="button"
