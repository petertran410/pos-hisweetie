...
export type StockConditionType = "normal" | "damaged" | "near_expiry";

export interface ConditionSelectableItem {
  rowId: string;
  product?: { id: number };
  productId?: number;
...
export type ConditionSelectableUpdates = Omit<
  Partial<ConditionSelectableItem>,
  "conditionType"
> & { conditionType?: StockConditionType };
...
export function ItemConditionSelector({
  ...
  lotPriorityVerb = "bán",
  usePortalForLotDropdown = false,
}: {
  item: ConditionSelectableItem;
  damagedAvailable: number;
  nearExpiryAvailable: number;
  branchId?: number;
  onUpdateItem: (rowId: string, updates: ConditionSelectableUpdates) => void;
  posOnly?: boolean;
  actionLabel?: string;
  lotPriorityVerb?: string;
  usePortalForLotDropdown?: boolean;
}) {
  ...
  return (
    <>
      ...
      {isNearExpiry && productId != null && (
        <NearExpiryLotSelect
          productId={productId}
          branchId={branchId}
          value={item.soldExpiryDate}
          posOnly={posOnly}
          lotPriorityVerb={lotPriorityVerb}
          usePortal={usePortalForLotDropdown}
          onChange={(expiryDate) =>
      ...
      {hasNearExpiry && (
        <ConditionButton
          ...
          title={
            isDamaged
              ? "Đang chọn hàng bục rách — bỏ chọn bục rách để chuyển sang cận date"
              : isNearExpiry
                ? "Đang chọn hàng cận date — bấm để bỏ chọn"
                : `${actionLabel} cận date (còn ${nearExpiryAvailable})`
          }
        />
      )}
      ...
      {hasDamaged && (
        <ConditionButton
          ...
          title={
            isNearExpiry
              ? "Đang chọn hàng cận date — bỏ chọn cận date để chuyển sang bục rách"
              : isDamaged
                ? "Đang chọn hàng bục rách — bấm để bỏ chọn"
                : `${actionLabel} hàng bục rách (còn ${damagedAvailable})`
          }
        />
      )}
    </>
...
function NearExpiryLotSelect({
  ...
  branchId?: number;
  value?: string | null;
  onChange: (expiryDate: string | null | undefined) => void;
  posOnly: boolean;
  lotPriorityVerb: string;
  usePortal: boolean;
}) {
  ...
  const { data, isLoading } = useNearExpiryLots(productId, branchId, posOnly);
  const lots = useMemo(() => data?.data || [], [data]);
  const panelPosition = useDropdownPosition(
    open && usePortal,
    triggerRef,
    288,
    256,
  ...
  const selectedLot = hasSelection
    ? lots.find((l) => toDateKey(l.expiryDate) === selectedKey)
    : undefined;
  ...
  const lotOptions = (
    <div
      ...
      className={
        usePortal
          ? "fixed z-[10000] bg-white border rounded-lg shadow-lg overflow-y-auto p-1"
          : "absolute z-50 right-0 top-full mt-1 w-64 bg-white border rounded-lg shadow-lg max-h-72 overflow-y-auto p-1"
      }
      style={
        usePortal && panelPosition
          ? {
              left: panelPosition.left,
              top: panelPosition.top,
              width: panelPosition.width,
              maxHeight: panelPosition.maxHeight,
              ...(panelPosition.dropUp
    ...
    >
      {showPriority && (
        <div className="flex items-center gap-1 px-2 py-1 mb-0.5 text-[11px] text-red-600 bg-red-50 rounded">
          ...
          <span>
            Lô tô đỏ là cận date xa nhất, nên {lotPriorityVerb} trước
          </span>
        </div>
      )}
      {isLoading ? (
        ...
        lots.map((lot) => {
          ...
          const active = hasSelection && key === selectedKey;
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => {
                onChange(lot.expiryDate ?? null);
                setOpen(false);
              }}
              title={
                priority
                  ? `Lô cận date xa nhất — đã nằm kho ${age}, nên ${lotPriorityVerb} trước`
                  : undefined
              }
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        title={
          hasSelection
            ? `Đang chọn lô NSX ${formatLotLabel(value)} — bấm để đổi lô`
      ...
      >
        <span className="text-xs font-medium whitespace-nowrap">
          {hasSelection ? `NSX ${formatLotLabel(value)}` : "Chọn lô NSX"}
        </span>
      ...
      {open &&
        (usePortal
          ? panelPosition &&
            ...
            createPortal(lotOptions, document.body)
          : lotOptions)}
    </div>
  );
}
