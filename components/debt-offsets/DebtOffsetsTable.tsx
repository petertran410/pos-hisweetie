...
export function DebtOffsetsTable({ filters }: DebtOffsetsTableProps) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto bg-white w-[60%] mt-4 mr-4 mb-4 border rounded-xl">
      <div className="flex items-center justify-between p-4 border-b bg-white">
        <input
          type="text"
          placeholder="Tìm theo mã phiếu trả hàng, tên KH..."
          value={search}
          onChange={(e) => {
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="text-center py-8 text-gray-500">
                  Không có bản ghi cấn trừ công nợ
                </td>
              </tr>
            ) : (
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      {item.refundConfirmedByName || item.creator?.name || "-"}
                    </td>
                    <td className="px-4 py-3">
