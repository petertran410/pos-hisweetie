...
import { useCreateInventoryPromoCheck } from "@/lib/hooks/useInventoryPromoChecks";
...
export function InventoryPromoCheckForm({ onClose }: Props) {
  const { selectedBranch } = useBranchStore();
  const createCheck = useCreateInventoryPromoCheck();
    ...
    setItems((prev) => [
      {
        ...
        currentPromo: inv ? Number(inv.promoQuantity || 0) : 0,
  ...
  const handleSubmit = () => {
    ...
    for (const item of items) {
      const promo = parseInt(item.promoQuantity) || 0;
      const available =
        item.onHand - item.currentDamaged - item.currentNearExpiry;
      if (promo > available) {
        alert(
          `${item.productName}: SL khuyến mãi (${promo}) vượt quá hàng khả dụng (${available})`
        );
        return;
      }
    }
    ...
    createCheck.mutate(
      {
        ...
        items: items.map((i) => ({
          ...
          promoQuantity: parseInt(i.promoQuantity) || 0,
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-5xl h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <h2 className="text-base font-semibold">
            Tạo phiếu kiểm hàng khuyến mãi
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-auto p-4">
                  ...
                  return (
                    <button
                      ...
                      <span className="text-xs text-gray-500">
                        ...
                        {" · "}
                        KM:{" "}
                        {inv ? Number(inv.promoQuantity || 0).toLocaleString() : 0}
                      </span>
                    </button>
                  );
                })}
              ...
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right">KM hiện tại</th>
                    <th className="px-3 py-2 text-center">KM mới</th>
                    <th className="px-3 py-2 text-center">Hàng thường còn</th>
                ...
                <tbody>
                  {items.map((item) => {
                    ...
                    const available =
                      item.onHand - item.currentDamaged - item.currentNearExpiry;
                    const remainingNormal = available - promo;
                    const isOverflow = promo > available;

                    return (
                      <tr key={item.productId} className="border-t">
                        <td className="px-3 py-2 text-xs">{item.productCode}</td>
                        ...
                        <td className="px-3 py-2 text-right text-gray-400">
                          {item.currentPromo.toLocaleString()}
                        </td>
                        <td className="px-3 py-2 text-center">
                          <input
                            type="text"
                            value={item.promoQuantity}
                            onChange={(e) =>
                              updateItem(
                                item.productId,
                                "promoQuantity",
                        ...
                        <td
                          ...
                          {isOverflow
                            ? "Vượt quá!"
                            : remainingNormal.toLocaleString()}
                        </td>
          ...
          {items.length === 0 && (
            <div className="text-center py-12 text-gray-400 text-sm">
              Tìm và thêm sản phẩm cần phân bổ cho khuyến mãi ở trên
            </div>
          )}
        </div>
