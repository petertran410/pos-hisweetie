...
export function PackingHangForm({
  ...
  >(() => {
    ...
    packingHang?.invoices?.forEach((i: any) => {
      if (i.invoice) {
        init[i.invoiceId] = {
          id: i.invoice.id,
          code: i.invoice.code,
  ...
  const [note, setNote] = useState(packingHang?.note || "");
  ...
  const {
    ...
    (files) =>
      uploadPackingHangImages(
        files,
        docType === "consignment" ? "ky-gui" : "dong-hang"
      ),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Còn ảnh đang upload → chặn lưu để không mất ảnh khỏi phiếu.
    if (hasPendingImages) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 sm:p-4">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {(images.length > 0 || pendingImages.length > 0) && (
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                    {/* Ảnh đang upload — hiện ngay bằng blob local */}
                    {pendingImages.map((p) => (
      ...
      {showQrModal && (
        <QrUploadModal
          subfolder={docType === "consignment" ? "ky-gui" : "dong-hang"}
