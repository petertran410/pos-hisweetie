...
function getDateRangeFromPreset(
  ...
  switch (preset) {
    ...
    case "last_week": {
      ...
      return {
        from: s,
        to: new Date(e.getFullYear(), e.getMonth(), e.getDate(), 23, 59, 59),
      };
    }
    case "last_7_days":
...
export function VehicleShipmentSidebar({ filters, setFilters }: Props) {
  ...
  // Đẩy chi nhánh đã chọn lên filters.
  useEffect(() => {
    setFilters({
  ...
  const borderGateOptions = useMemo<SimpleOption[]>(
    () =>
    ...
    <aside className="w-72 border m-4 rounded-xl custom-sidebar-scroll bg-white shadow-xl flex flex-col">
      ...
      <div className="p-4 space-y-3 overflow-y-auto flex-1">
        ...
        <div>
          ...
          <FilterSearchableSelect
            options={borderGateOptions}
            value={borderGateValue}
            placeholder="Tất cả cửa khẩu"
