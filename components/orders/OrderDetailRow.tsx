...
export function OrderDetailRow({ orderId, colSpan }: OrderDetailRowProps) {
  ...
  useLayoutEffect(() => {
    ...
    // Tìm scroll container gần nhất (overflow-auto/scroll trên trục X)
    let scrollEl: HTMLElement | null = wrapperRef.current.parentElement;
    while (scrollEl) {
      const ox = window.getComputedStyle(scrollEl).overflowX;
    ...
    <>
      <tr>
        <td colSpan={colSpan} className="p-0 bg-gray-50">
          <div
            ref={wrapperRef}
            className="sticky left-0 py-2 px-2 bg-gray-50"
