...
export default function CanTruCongNoPage() {
  ...
  const customerFilters = { ...sidebarFilters, ...(codeParam ? { search: codeParam } : {}), refundType: "debt_offsets", status: 4 };
  const supplierFilters = { ...sidebarFilters, ...(codeParam ? { search: codeParam } : {}) };
    ...
    <PagePermissionGuard resource="return_orders" action="view">
      <div className="flex h-full border-t" style={{ borderColor: "var(--dt-border)" }}>
        ...
        <div className="flex-1 flex flex-col overflow-hidden mt-4 mr-4 mb-4">
          <div className="flex gap-1 border-b bg-white rounded-t-[8px] px-4 pt-2" style={{ borderColor: "var(--dt-border)" }}>
            {TABS.map((tab) => (
              <button key={tab.value} onClick={() => setActiveTab(tab.value)} className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${activeTab === tab.value ? "border-brand text-brand" : "border-transparent"}`} style={activeTab === tab.value ? undefined : { color: "var(--dt-text-secondary)" }}>
                {tab.label}
              </button>
            ))}
          </div>
          <div className="flex-1 overflow-hidden flex">
            {activeTab === "customer" ? <DebtOffsetsTable filters={customerFilters} /> : <SupplierDebtOffsetsTable filters={supplierFilters} />}
          </div>
        </div>
      </div>
    </PagePermissionGuard>
